...
(function (root) {
  ...
  function styles(used, coreStyles, extra) {
    ...
    used.forEach(function (si) {
      var st = (extra && Object.prototype.hasOwnProperty.call(extra, si)) ? extra[si] : (coreStyles[si] || {});
      var nf = fmt(st), fo = font(st), fi = fill(st);
      var al = '', apply = '';
  ...
  // extra: optional map of synthetic style keys -> {b,i,fc,bg,fmt,ha,ind,wrap}; a cell's s may name one of them
  function pack(sheets, extra) {
    var used = {};
    sheets.forEach(function (S) { Object.keys(S.rows).forEach(function (r) { S.rows[r].forEach(function (cd) { used[cd.s] = 1; }); }); });
    var st = styles(Object.keys(used), root.MODEL_CORE.styles, extra);
    var files = [
