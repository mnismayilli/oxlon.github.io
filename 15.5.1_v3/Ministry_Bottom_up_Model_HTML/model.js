...
(function (root) {
  ...
  function Model(core, parts, tplFactory) {
    ...
    this.range = function (gi) {
      ...
      return (rcache[gi] = { isRange: true, ids: ids, rows: R[3] - R[1] + 1, cols: R[4] - R[2] + 1 });
    };
  ...
  Model.prototype.evalCell = function (id) {
    ...
    try {
      ...
      else if (v && v.isRange) v = X.E('#VALUE!');
  ...
  Model.prototype.recalcAll = function (overrides) {
    for (var i = 0; i < this.order.length; i++) {
      var id = this.order[i];
      this.V[id] = (overrides && overrides.has(id)) ? overrides.get(id) : this.fixed.has(id) ? this.fixed.get(id) : this.evalCell(id);
    }
  };
